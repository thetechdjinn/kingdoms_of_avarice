...
async function fetchAvailableSpells(): Promise<void> {
  try {
    ...
    availableSpells = (data.spells || []).map((s: Record<string, unknown>) => ({
      ...
      manaCost: (s.manaCost ?? 0) as number,
      minDamage: (s.minDamage ?? 0) as number,
      maxDamage: (s.maxDamage ?? 0) as number,
      minHealing: (s.minHealing ?? 0) as number,
      maxHealing: (s.maxHealing ?? 0) as number,
      scalingPerLevel: (s.scalingPerLevel ?? 0) as number,
      damageScalingFactor: (s.damageScalingFactor ?? 0) as number,
      healingScalingFactor: (s.healingScalingFactor ?? 0) as number,
      maxScalingLevel: (s.maxScalingLevel ?? 0) as number,
      hitsPerCast: (s.hitsPerCast ?? 1) as number,
      levelRequired: (s.levelRequired ?? 1) as number,
    }));
  } catch (e) {
