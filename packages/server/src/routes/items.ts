...
export function setupItemRoutes(app: Express): void {
  ...
  app.post('/api/items/templates', requireDeveloper, async (req: Request, res: Response) => {
    try {
      ...
      if (max_in_world !== undefined && max_in_world !== null && (typeof max_in_world !== 'number' || !Number.isInteger(max_in_world) || max_in_world < 1)) {
  ...
  app.put('/api/items/templates/:id', requireDeveloper, async (req: Request, res: Response) => {
    try {
      ...
      // Auto-regenerate short_desc from name when name changes and no explicit short_desc provided,
      // but only if the existing short_desc matches the auto-generated form (preserve custom descriptions)
      const updates = { ...req.body };
      if (updates.name && !updates.short_desc) {
        const autoGeneratedFromOld = withArticle(existing.name.toLowerCase());
        if (existing.short_desc === autoGeneratedFromOld) {
          updates.short_desc = withArticle(updates.name.toLowerCase());
        }
      }

      const template = await itemRepo.updateTemplate(id, updates);
      res.json({ success: true, template });
    } catch (error) {
