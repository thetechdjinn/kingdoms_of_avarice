 ...
 */
function deductCopperFromWallet(
  currency: Currency,
  copperAmount: number
  ...
  if (remaining > 0) {
    for (const { field, copperValue } of DENOMINATIONS_LOW_TO_HIGH) {
      if (wallet[field] > 0 && copperValue >= remaining) {
        // Break one coin of this denomination
        spent[field] += 1;
        const change = copperValue - remaining;
        wallet[field] -= 1;
        remaining = 0;

        // Give change back as lower denominations
        ...
        for (let i = DENOMINATIONS_LOW_TO_HIGH.length - 1; i >= 0; i--) {
          ...
          if (lower.copperValue <= changeLeft) {
            ...
            if (changeCoins > 0) {
              // Change goes back into wallet — reduce what we're spending
  ...
  // Return only non-zero deductions
